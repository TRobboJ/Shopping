...
export default function StoreView(props) {
  ...
  const { menuIsOpen, query, minPrice, maxPrice, ratingQuery } = useSelector(
    (state) => state.filter
  );
  const iconSize = `30px`;
...
function getTopBarInfo(
  ...
  if (!productArray) {
    return `Error collecting products`;
  }
  let output = [];
  ...
  if (filterOutput.length > 1) {
    concatFilterOutput = `${filterOutput
      .slice(0, -1)
      .join(", ")} and ${filterOutput.slice(-1)}`;
  }
  if (filterOutput.length === 1) {
    concatFilterOutput = filterOutput;
  }
  ...
  output.push(
    <p className={styles.queries}>
      {filtersExist ? `Showing results for ${concatFilterOutput}.` : noFilters}
    </p>
  );

...
export default function ProductSidebar(props) {
  ...
  const renderCategories = categories.map((category, i)=> {
    return (
      <li key={i} onClick={filterQueryHandler}>{category.charAt(0).toUpperCase() + category.slice(1)}</li>
    )
  })
  ...
  return (
    ...
    <div className={styles.sidebar}>
      ...
      {sidebarCloseIcon}
        ...
        <div className={styles.header}><label>Search</label></div>
        <div className={styles.content}>
          <div className={styles.query_input}>
            <input type='text' placeholder='Search titles...' onChange={filterQueryHandler}/>
            </div>
        </div>
        <div className={styles.header}><label>Categories</label></div>
        <div className={styles.content}>
          <ul>
            {renderCategories}
          </ul>
        </div>
        <div className={styles.header}><label>Price</label></div>
        <div className={styles.content}>
        ...
        <div className={styles.header}><label>Reset</label></div>
        <div className={styles.content}>
        <div className={styles.reset}>
          <input type='reset' value='Reset' onClick={clearAllFiltersHandler}/>
          </div>
        </div>
